...
# create website
web_dir = os.path.join('./results/DESIGNATE/')
...
class GaGan(object):
    ...
    def control_car_from_csv(self):
        ...
        for row in rows:
            ...
            self.GaGan.client.simSetCameraPose("0", pose)
            self.GaGan.client.simSetCameraPose("1", pose)
            time.sleep(1)
    ...
    def searchAlgo(self, n_gen, pop_size, numSet):
        path = "app/search_sets/mars/search2_feat/set" + str(numSet)
        ...
        with open('app/ga_results.csv', 'w', newline='') as csvfile:
            fieldnames = ['img', 'individual', 'entry', 'rot', 'F', 'PixAcc']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()
            for pos in MyProblem.archive:
                writer.writerow(pos)
                # shutil.copy(pos['img'], './results/DESIGNATE/archive/'+os.path.basename(pos["img"]))
...
class GanProblem(pymoo.problems.multi.Problem):
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        path = "./results/DESIGNATE/"
        os.makedirs(path, exist_ok = True) 
        F = []
        import psutil
        
        # with open('app/archive.txt', "r") as f:
        #     file = f.read().split('\n')
        archive2 = []
        archive_features = []
        ...
        for x in X:
            ...
            if True:
                ...
                # elif (f_simulated-f) > 0.01 and not math.isnan(float(f)):
                #     f = 2
                #     f2 = 2
                #     print('image not realistic')
                    ...
                    if(len(archive2) == 0):
                        ...
                        if(type_distance != 'PixelAccuracy'):
                            ...
                            archive_features.append(features)     
                        f2 = 1               
                        self.archive.append({'img': save_path, 'individual': (x[0], x[1]), 'entry': int(x[0]*len(self.rows)), 'rot': int(x[1]*360), 'F': (f, random.uniform(12.0, 30.0)), 'PixAcc': 0})
                        ...
                        if(closest_distance > threshold_diversity): #  if PixelAccuracy use '<' if Euclidean use '>'
                            ...
                            closest_image_path = archive2[closest_index]
                            pred_closest = predict_img(closest_image_path)
                            label_DNN2 = generate_dnn_mask(closest_image_path.replace('R_', 'L_'))
                            

                            f_closest = compute_miou(compute_iou(pred_closest, label_DNN2, 4))

                            if(f_closest > f):
                                self.archive[closest_index] = {'img': save_path, 'individual': (x[0], x[1]), 'entry': int(x[0]*len(self.rows)), 'rot': int(x[1]*360), 'F': (f, random.uniform(12.0, 30.0)), 'PixAcc': 0}
