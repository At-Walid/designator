...
# create website
web_dir = os.path.join("./results/DESIGNATE_NOGAN/")
...
class GaGan(object):
    ...
    def __init__(self):
        ...
        self.airsim = airsim
        self.dir = "./results/DESIGNATE_NOGAN/"
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        for x in X:
            ...
            if True:
                ...
                if not os.path.isfile(simulated_path):
                    t1 = time.time()
                ...
                rgb_pred = Image.fromarray(generate_rgb_mask(pred2), 'RGB')
                ...
                # Image.fromarray(pred.astype(np.uint8)).save(os.path.join(path, "P_" + str(x[0]) + "_" + str(x[1]) + ".png"))

                # print('fitnesses', f, f_simulated)
                # target_color = [219, 196, 184]
                # color_proportion = compute_color_proportion(simulated_path, target_color)
                proportion = compute_all_color_proportion(label_path)
                
                

                # f = miou(pred, label_DNN, 19)
                # f_simulated = miou(pred_simulated, label_DNN, 19)
                ...
                elif math.isnan(float(f_simulated)):
                    f = 2
                    f2 = 2
                    print('image not realistic')
                else:
                    f = f_simulated
                    # f2 = closest_distance
                    if(type_distance != 'PixelAccuracy'):
                        input_image_path = simulated_path
                    else:
                        input_image_path = simulated_path
                    if(len(archive2) == 0):
                        ...
                        if(type_distance != 'PixelAccuracy'):
                            ...
                            archive_features.append(features)     
                        f2 = 1               
                        self.archive.append({'img': simulated_path, 'individual': (x[0], x[1]), 'entry': int(x[0]*len(self.rows)), 'rot': int(x[1]*360), 'F': (f, f2), 'PixAcc': 0})
                        ...
                        if(closest_distance > threshold_diversity): #  if PixelAccuracy use '<' if Euclidean use '>'
                            ...
                            closest_image_path = archive2[closest_index]
                            if(type_distance != 'PixelAccuracy'):
                                pred_closest = np.array(predict(self.DNN, closest_image_path))
                                label_GAN2, label_DNN2 = mask_label(closest_image_path.replace('R_', 'L_'))
                            else:
                                pred_closest = np.array(predict(self.DNN, closest_image_path.replace('R_', 'R_')))
                                label_GAN2, label_DNN2 = mask_label(closest_image_path.replace('R_', 'L_'))

                            f_closest = miou(pred_closest, label_DNN2, 19)

                            if(f_closest > f):
                                self.archive[closest_index] = {'img': simulated_path, 'individual': (x[0], x[1]), 'entry': int(x[0]*len(self.rows)), 'rot': int(x[1]*360), 'F': (f, f2), 'PixAcc': 0}
                                print('replaced in archive')
                ...
                with open(r'./archive.txt', 'w') as fp:
        ...
        if(type_distance != 'PixelAccuracy'):
            ...
            for i in range(len(X)):
                x = X[i]
                img_path = path + "S_" + str(x[0]) + "_" + str(x[1]) + ".png"
